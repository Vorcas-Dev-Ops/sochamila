 ...
 */
async function generateJerseyImages(options) {
...
${customInstructions ? `\nADDITIONAL INSTRUCTIONS:\n${customInstructions}` : ""}
    `.trim();
        ...
        const uploadResponse1 = await imagekit_1.imagekit.upload({
            file: imageData1,
            fileName: `jersey-front-${Date.now()}.png`,
            folder: "/jersey-designs",
            tags: ["jersey", "front-view", "dall-e-3"],
        });
        const uploadResponse2 = await imagekit_1.imagekit.upload({
            file: imageData2,
        ...
        if (err?.status === 429 ||
            ...
            const rateError = new Error(`OpenAI rate limit exceeded: ${err?.message || "Please try again in a minute."}`);
