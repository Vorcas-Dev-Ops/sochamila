...
exports.verifyPANWithCashfree = verifyPANWithCashfree;
exports.verifyAadhaarWithCashfree = verifyAadhaarWithCashfree;
exports.verifyGSTINWithCashfree = verifyGSTINWithCashfree;
exports.verifyBankAccountWithCashfree = verifyBankAccountWithCashfree;
exports.verifyUPIWithCashfree = verifyUPIWithCashfree;
exports.verifyVendorKYCWithCashfree = verifyVendorKYCWithCashfree;
exports.isKYCValid = isKYCValid;
exports.getKYCErrorMessages = getKYCErrorMessages;
...
===================================================== */
async function verifyPANWithCashfree(pan) {
    try {
        ...
        const response = await axios_1.default.post(panUrl, panRequestBody, {
...
===================================================== */
async function verifyAadhaarWithCashfree(aadhaar) {
    try {
        // Skip API call if SKIP_KYC is enabled
        if (SKIP_KYC) {
        ...
        const response = await axios_1.default.post(aadhaarUrl, {
...
===================================================== */
async function verifyGSTINWithCashfree(gstin) {
    try {
        // Skip API call if SKIP_KYC is enabled
        if (SKIP_KYC) {
            console.log('[CASHFREE] SKIP_KYC_VERIFICATION is enabled, using format validation only');
        ...
        const response = await axios_1.default.post(gstUrl, gstRequestBody, {
            headers: await getCashfreeHeaders(),
        });
        const data = response.data;
...
===================================================== */
async function verifyBankAccountWithCashfree(accountNumber, ifsc) {
    try {
        // Skip API call if SKIP_KYC is enabled
        if (SKIP_KYC) {
            console.log('[CASHFREE] SKIP_KYC_VERIFICATION is enabled, using format validation only');
        ...
        const response = await axios_1.default.post(bankUrl, requestBody, {
            headers: await getCashfreeHeaders(),
        });
        const data = response.data;
...
===================================================== */
async function verifyUPIWithCashfree(upiId) {
    try {
        // Skip API call if SKIP_KYC is enabled
        if (SKIP_KYC) {
            console.log('[CASHFREE] SKIP_KYC_VERIFICATION is enabled, using format validation only');
        ...
        const response = await axios_1.default.post(upiUrl, {
            upi_vpa: cleaned,
        }, {
            headers: await getCashfreeHeaders(),
        });
...
===================================================== */
async function verifyVendorKYCWithCashfree(data) {
    const result = {
    ...
    if (data.payoutMethod === 'BANK' && data.accountNumber && data.ifsc) {
        result.bankAccount = await verifyBankAccountWithCashfree(data.accountNumber, data.ifsc);
...
function verifyAadhaarFormat(aadhaar, skipChecksum = false) {
    const cleaned = aadhaar.trim().replace(/\s/g, '');
    if (!cleaned) {
...
function verifyGSTINFormat(gstin) {
    if (!gstin || !gstin.trim()) {
        return { valid: true, message: 'GSTIN is optional', verified: true };
    }
    const cleaned = gstin.trim().toUpperCase();
...
function verifyBankAccountFormat(accountNumber, ifsc) {
...
function verifyUPIFormat(upiId) {
    if (!upiId || !upiId.trim()) {
...
===================================================== */
function isKYCValid(verification) {
...
===================================================== */
function getKYCErrorMessages(verification) {
    const errors = [];
    if (!verification.pan.valid) {
