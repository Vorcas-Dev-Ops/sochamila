...
import { generateJerseyImages as generateJerseyImagesService } from "./jersey.generate.service";
...
export async function generateJerseyImages(
  ...
  try {
    ...
    // Generate images
    const result = await generateJerseyImagesService({
      prompt,
      customInstructions,
    });
    ...
    if (
      ...
      return res.status(429).json({
        success: false,
        error:
          "Too many requests. OpenAI rate limit exceeded. Please try again in 1-2 minutes.",
        retryAfter: "60s",
      });
    }
